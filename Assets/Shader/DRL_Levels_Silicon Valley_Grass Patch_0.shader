Shader "DRL/Levels/Silicon Valley/Grass Patch" {
	Properties {
		[Header(MAIN MAPS (UV1))] [Space(10)] _GrassAlbTex ("Albedo (RGB) Opacity (A)", 2D) = "white" {}
		[NoScaleOffset] _GrassMetTex ("Smoothness (A)", 2D) = "white" {}
		[NoScaleOffset] [Normal] _GrassNorTex ("Normal (RGB)", 2D) = "bump" {}
		[Space(10)] [Header(PROPERTIES)] [Space(10)] _Color ("Color", Vector) = (1,1,1,1)
		_Cutoff ("Cutout", Range(0, 1)) = 0.5
		_Smoothness ("Smoothness", Range(0, 1)) = 0.5
		_GridSize ("Grid Size", Range(0, 100)) = 10
		_Softness ("Softness", Range(0, 1)) = 1
		[Header(HEIGHT)] [Space(10)] _HeightMin ("Min", Range(0, 2)) = 0.5
		...
		[Space(10)] [Space(20)] [Header(EFFECTS MAPS)] [Space(10)] _FxColor ("Color", Range(0, 1)) = 0.25
		_FxElevation ("Elevation", Float) = 5
		_FxUVWidth ("UV Width", Float) = 20
		_FxUVHeight ("UV Height", Float) = 20
		[NoScaleOffset] _FxCMETex ("Color (R) Mask (G) Elevation (B)", 2D) = "black" {}
	}
