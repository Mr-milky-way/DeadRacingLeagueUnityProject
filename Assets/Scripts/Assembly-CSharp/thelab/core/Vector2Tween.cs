...
namespace thelab.core
{
	...
	{
		public Vector2Tween(object p_target, string p_property, Vector2 p_to, float p_duration, float p_delay, Easing p_easing, bool p_threaded)
			: base(p_target, p_property, p_to, p_duration, p_delay, p_easing, p_threaded)
		{
		}

		public Vector2Tween(object p_target, string p_property, Vector2 p_to, float p_duration, float p_delay, Easing p_easing)
			: this(p_target, p_property, p_to, p_duration, p_delay, p_easing, p_threaded: false)
		{
		}

		public Vector2Tween(object p_target, string p_property, Vector2 p_to, float p_duration, Easing p_easing)
			: this(p_target, p_property, p_to, p_duration, 0f, p_easing, p_threaded: false)
		{
		}

		public Vector2Tween(object p_target, string p_property, Vector2 p_to, Easing p_easing)
		...
		{
			Vector2 vector = from + (to - from) * easing(p_ratio);
			switch (type)
			{
			case Target.Type:
