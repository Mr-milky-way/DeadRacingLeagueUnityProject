...
public class postVHSPro : MonoBehaviour
{
	...
	private void OnRenderImage(RenderTexture src, RenderTexture dest)
	{
		...
		if (screenLinesNum <= 0f)
		{
			screenLinesNum = src.height;
		}
		if ((tapeNoiseOn || filmgrainOn || lineNoiseOn) && (texTape == null || (float)texTape.height != Mathf.Min(noiseLinesNum, screenLinesNum)))
		{
			int num = (int)Mathf.Min(noiseLinesNum, screenLinesNum);
			int width = (int)((float)num * (float)src.width / (float)src.height);
			Object.DestroyImmediate(texTape);
			texTape = new RenderTexture(width, num, 0);
		...
		mat1.SetFloat("screenLinesNum", screenLinesNum);
