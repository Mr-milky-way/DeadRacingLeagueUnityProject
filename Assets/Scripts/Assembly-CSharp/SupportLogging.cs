...
public class SupportLogging : MonoBehaviour
{
	...
	private void LogBasics()
	{
		...
		stringBuilder.AppendFormat("SupportLogger Info: PUN {0}: ", "1.98");
	...
	{
		Debug.Log("SupportLogger OnJoinedRoom(" + PhotonNetwork.room?.ToString() + "). " + PhotonNetwork.lobby?.ToString() + " GameServer:" + PhotonNetwork.ServerAddress);
	}

	public void OnCreatedRoom()
	{
		Debug.Log("SupportLogger OnCreatedRoom(" + PhotonNetwork.room?.ToString() + "). " + PhotonNetwork.lobby?.ToString() + " GameServer:" + PhotonNetwork.ServerAddress);
	}
