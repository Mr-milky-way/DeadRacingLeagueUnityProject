...
namespace drl.sim
{
	public class FCPitchProcess : FCManeauverProcess
	{
		...
		protected override void OnPIDUpdate(PID p_pid)
		{
			...
			p_correction = base.fc.PitchCorrection(p_pid.constants.p);
			_ = base.fc.drone;
			float p_current = base.fc.sensor.gyro.velocity.x * ((float)Math.PI / 180f);
			float p_target = base.fc.signal.pitch * p_correction;
