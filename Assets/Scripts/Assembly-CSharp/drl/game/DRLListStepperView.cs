...
namespace drl.game
{
	public class DRLListStepperView<T> : ListStepperView<T>
	{
		public Image arrow;
		...
		private MonoActivity m_preview_timer;
		...
		protected override void OnState(string p_state)
		{
			...
			RefreshPreview(p_state);
		}

		protected void RefreshPreview(string p_state)
		{
			if (this == null || base.gameObject == null || !previewUpContainer || !previewDownContainer)
			{
				return;
			}
			Text text = null;
			...
			m_preview_timer = RunOnce(delegate
			{
				if (base.validContext)
				{
