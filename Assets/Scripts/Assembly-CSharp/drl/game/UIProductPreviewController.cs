...
namespace drl.game
{
	public class UIProductPreviewController : Controller<DRLApp>
	{
		public DRLStoreProductData currentProduct;

		private bool openedFromGarage;
		...
		private UIStoreProductItemView m_selection;
		...
		public DroneRigData customDroneRig;
		...
		public UIProductPreviewView view => AssertLocal<UIProductPreviewView>("view");
		...
		public UIStoreProductItemView selection
		{
			get
			{
				return m_selection;
		...
		public override void OnNotification(string p_event, UnityEngine.Object p_target, params object[] p_data)
		{
			...
			{
			case "ui.screen@open":
				Debug.Log("UIProductPreviewController > Screen.Open - <color=blue>" + view.data.name + " is the current data</color>");
				if (p_data[0] as UIScreen != view.screen)
				{
					break;
				}
				currentProduct = view.data;
				...
				EnableGarageScene(p_enable: true, MoveDroneToGarageScene);
				if (view.rigdata.esc == "E-000")
				{
					DroneRigData originalByFrame = model.GetOriginalByFrame(view.rigdata.frame);
					if ((bool)originalByFrame && view.rigdata.esc != originalByFrame.esc)
					{
						RefreshDronePart(originalByFrame.esc);
					}
				}
				view.testSkin = FindDronePartInLibrary("SK-000") as DroneSkin;
				view.SetCardsData();
				ResetViewerFocus();
				break;
			...
			case "garage.edit.item@click":
			{
				...
				{
					view.selectedCard = uICardButtonGarageEditItem;
					_ = uICardButtonGarageEditItem.data.guid;
					Debug.Log("<color=green>Current Skin:</color> " + view.rigdata.skinFrame);
					ChangePart(uICardButtonGarageEditItem);
					Debug.Log("<color=green>New Current Skin:</color> " + view.rigdata.skinFrame);
					Debug.Log("<color=green>New Current Prop:</color> " + view.rigdata.prop);
			...
			case "garage.edit.back@click":
			{
				...
				DroneRigData rigdata = view.rigdata;
				model.GetDroneSpecData(rigdata);
			...
			{
				DRLStoreProductData productData = currentProduct;
				if (productData == null)
				{
					break;
				}
				base.app.model.service.BuyProduct(productData, delegate(bool p_success, string p_error)
				{
					if (!p_success)
					{
						base.app.view.ui.dialog.Open(DialogType.Error, "PURCHASE FAILED", p_error.ToUpper(), new string[1] { "OK" });
					}
					else
					{
						Debug.Log("UIProductPreviewController> StoreItemBuyClick / Success - Adding Items\n" + string.Join("\n", productData.items));
		...
		protected void OnItemBuyComplete(DRLStoreProductData p_product)
		{
			...
			base.app.view.ui.dialog.Open(DialogType.Info, "PURCHASE SUCCESSFUL", "THANK YOU FOR YOUR PURCHASE. \nYOU CAN VIEW YOUR " + text + " IN THE GARAGE.", new string[2] { "GARAGE", "CLOSE" }, null, "", delegate(string p_id, int p_option)
			{
				if (p_option == 1)
				{
					...
					base.app.model.storage.PreloadDroneBundleData(null, defaultStoreRig, p_ingame: false, delegate
					{
						OpenGarageEdit(defaultStoreRig, view.selectedCard.data);
					});
				}
			});
		}
		...
		public void SetTab(string p_id)
		{
			...
			goto IL_0185;
			IL_0185:
			SetViewerFocus(p_id2, p_center: true);
		...
		private void ResetViewerFocus()
		{
			...
			orbit.anchor = new Vector3(0f, -100f, 0f);
		}
		...
		protected void CaptureScreenshot(DroneScreenshotData p_dsd)
		{
			...
			Activity.RunOnce(delegate
			{
				...
				capture.CaptureAsync(640, 640, view.thumbnailDrone.transform, p_dsd, p_smooth: true, p_preview: false, p_mipmap: false, delegate
				{
				});
			}, 2f);
		}
		...
		protected void CheckAllDronePartsAgainstLibrary(DroneRigData p_original, DroneRigData p_oldRig)
		{
			...
			{
				Debug.LogWarning("UIProductPreviewController> had to replace motor because of incompatibility: " + p_oldRig.motor);
			}
			if (p_original.prop != p_oldRig.prop)
			{
				Debug.LogWarning("UIProductPreviewController> had to replace propeller because of incompatibility: " + p_oldRig.prop);
			}
			if (p_original.battery != p_oldRig.battery)
			{
				Debug.LogWarning("UIProductPreviewController> had to replace battery because of incompatibility: " + p_oldRig.battery);
			}
			if (p_original.attachment0 != p_oldRig.attachment0)
			{
				Debug.LogWarning("UIProductPreviewController> had to replace attachment because of incompatibility: " + p_oldRig.attachment0);
			}
			...
			{
				Debug.LogError("UIProductPreviewController> F-692 frame is not in the originals!");
				return;
			}
			...
			if (!CheckPartAgainstFrame(parts[1], p_frame))
			{
				...
				Debug.Log("UIProductPreviewController> Motor changed, compat = " + CheckPartAgainstFrame(parts[1], p_frame));
			...
			if (!CheckPartAgainstFrame(parts[2], p_frame) || !CheckPropAgainstMotor(parts[2], (DroneMotor)parts[1]))
			{
				...
				Debug.Log("UIProductPreviewController> Changed prop, compat = " + CheckPartAgainstFrame(parts[2], p_frame));
			...
			if (!CheckPartAgainstFrame(parts[4], p_frame))
			{
				...
				RefreshVideoAndDatasheet();
				Debug.Log("UIProductPreviewController> Battery changed, compat = " + CheckPartAgainstFrame(parts[4], p_frame));
				p_original.battery = originalByFrame.battery;
			}
			if (!CheckPartAgainstFrame(parts[5], p_frame))
			{
				RefreshDronePart(originalByFrame.esc);
				Debug.Log("UIProductPreviewController> Esc changed, compat = " + CheckPartAgainstFrame(parts[5], p_frame));
				p_original.esc = originalByFrame.esc;
			}
			if (!CheckPartAgainstFrame(parts[7], p_frame))
			{
				RefreshDronePart(originalByFrame.attachment0);
				Debug.Log("UIProductPreviewController> Changed attachment, compat = " + CheckPartAgainstFrame(parts[7], p_frame));
		...
		protected void ChangePart(UICardButtonGarageEditItem p_part)
		{
			...
			case "frames":
				...
				break;
			case "motors":
				rawData.motor = (rigdata.motor = dronePart.guid);
				RefreshDronePart(dronePart.guid);
				Notify("garage.edit.change-part");
				break;
			case "props":
				rawData.prop = (rigdata.prop = dronePart.guid);
				RefreshDronePart(dronePart.guid);
				break;
			case "lipos":
				rawData.battery = (rigdata.battery = dronePart.guid);
				RefreshDronePart(dronePart.guid);
				Notify("garage.edit.change-part");
				break;
			case "extras":
				...
				{
					rawData.antenna = (rigdata.antenna = dronePart.guid);
					RefreshDronePart(dronePart.guid);
					Notify("garage.edit.change-part");
				}
				else if (dronePart is DroneAttachment)
				{
					rawData.attachment0 = (rigdata.attachment0 = dronePart.guid);
					RefreshDronePart(dronePart.guid);
					Notify("garage.edit.change-part");
				}
				else if (dronePart is DroneRFCamera)
				{
					rawData.camera = (rigdata.camera = dronePart.guid);
					RefreshDronePart(dronePart.guid);
					Notify("garage.edit.change-part");
				}
				break;
			case "skins":
				rawData.skinFrame = (rigdata.skinFrame = dronePart.guid);
				RefreshDronePart(dronePart.guid);
			...
			DroneMotor p_check = base.app.model.storage.library.FindByGUID<DroneMotor>(rigdata.motor);
			DroneProp p_part2 = base.app.model.storage.library.FindByGUID<DroneProp>(rigdata.prop);
			base.app.model.storage.library.FindByGUID<DroneBattery>(rigdata.battery);
			if (!DRLStoreFilter.Filter(p_part2, p_check))
			{
				rigdata.prop = originalByFrame.prop;
				RefreshDronePart(originalByFrame.prop);
			}
			RefreshLibrary();
			RefreshVideoAndDatasheet();
			if (flag)
			{
				...
				{
					clonedOriginalbyFrame.guid = rigdata.guid;
					clonedOriginalbyFrame.color0 = rigdata.color0;
					clonedOriginalbyFrame.color1 = rigdata.color1;
					clonedOriginalbyFrame.color2 = rigdata.color2;
					clonedOriginalbyFrame.thumb0 = rigdata.thumb0;
					clonedOriginalbyFrame.thumb1 = rigdata.thumb1;
					...
					{
						clonedOriginalbyFrame.motor = ((string.IsNullOrEmpty(rigdata.motor) || rigdata.motor == "M-000") ? rawData.motor : rigdata.motor);
						clonedOriginalbyFrame.battery = ((string.IsNullOrEmpty(rigdata.battery) || rigdata.battery == "B-000") ? rawData.battery : rigdata.battery);
						clonedOriginalbyFrame.prop = ((string.IsNullOrEmpty(rigdata.prop) || rigdata.prop == "P-000") ? rawData.prop : rigdata.prop);
						clonedOriginalbyFrame.attachment0 = ((string.IsNullOrEmpty(rigdata.attachment0) || rigdata.attachment0 == "AT-000") ? rawData.attachment0 : rigdata.attachment0);
					}
					clonedOriginalbyFrame.skinFrame = ((string.IsNullOrEmpty(rigdata.skinFrame) || rigdata.skinFrame == "SK-000") ? rawData.skinFrame : rigdata.skinFrame);
					clonedOriginalbyFrame.physics = "PH-000";
					clonedOriginalbyFrame.Validate();
					view.rigdata = clonedOriginalbyFrame;
				}
				...
				MarkRigDirty();
				RefreshLibrary();
				CheckAllDronePartsAgainstLibrary(clonedOriginalbyFrame, rigdata);
			}
			RefreshCenterOfGravity();
			if (flag2)
			{
			...
			if (view.data.category == "props")
			{
				Debug.Log("This item is a prop");
		...
		protected void PropellersAnimation(float p_duration = 0f)
		{
			...
			m_propellersRotation = Run(delegate(float p_elapsed)
			{
				...
				for (int i = 0; i < 4; i++)
				{
					...
					{
						view.drone.body.frame.escs[i].motor.animation.cap.Rotate(-Vector3.forward * (num * 720f - angle));
					}
					else
					{
						view.drone.body.frame.escs[i].motor.animation.cap.Rotate(Vector3.forward * (num * 720f - angle));
		...
		protected void CreateTrailsAnimation()
		{
			...
			m_trailsObjectReference.transform.localScale = one;
		...
		protected void MoveDroneToGarageScene()
		{
			...
			if (gameObject == null)
			{
				Debug.LogError("UIProductPreviewController> Could not find drone-node in Garage scene");
				return;
			}
			view.viewerTransform.gameObject.SetActive(value: true);
		...
		protected void RefreshDronePart(string p_guid)
		{
			...
			{
				view.thumbnailDrone = base.app.model.storage.factory.ReplacePart(view.drone, p_guid);
			}
			else
			{
		...
		protected void CreateNewDrone()
		{
			...
			DroneRigData defaultRigData = view.defaultRigData;
			_ = openedFromGarage;
			Transform droneContainer = view.droneContainer;
			view.drone = base.app.model.storage.factory.Replace(defaultRigData, drone, droneContainer, droneContainer, p_async: false);
		...
		{
			view.productName = view.data.name;
		}
		...
		protected void OnDroneEvent(DroneEvent p_event)
		{
			...
			{
				Debug.LogError("UIProductPreviewController> got ready event from unknown drone");
			}
		}
		...
		{
			ClearFilter();
			if (view.filter0.index != 0)
			{
				_ = view.filter0list[view.filter0.index];
			}
			if (view.filter1.index != 0)
			{
				_ = view.filter1list[view.filter1.index];
			}
		}
		...
		protected void OpenGarageEdit(DroneRigData p_data, DRLAsset p_purchasedProduct, bool p_templateSelector = false)
		{
			view.viewerTransform.gameObject.SetActive(value: false);
			if (p_data == null)
			{
				Debug.LogWarning("UIProductPreviewController> Failed to Open Edit");
			}
			else if (!p_data.isLocked)
			{
				...
				uIGarageRigEditView.currentProduct = p_purchasedProduct;
			}
		}
	}
}
