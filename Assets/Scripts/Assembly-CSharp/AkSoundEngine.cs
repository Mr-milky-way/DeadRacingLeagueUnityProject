...
public class AkSoundEngine
{
	...
	public static void LocalOutput(AkMonitorErrorCode in_eErrorCode, string in_pszError, AkMonitorErrorLevel in_eErrorLevel, uint in_playingID, GameObject in_gameObjID)
	{
		...
		AkSoundEnginePINVOKE.CSharp_LocalOutput((int)in_eErrorCode, in_pszError, (int)in_eErrorLevel, in_playingID, akGameObjectID);
	}
	...
	public static AKRESULT QueryReflectionPaths(GameObject in_gameObjectID, ref Vector3 out_listenerPos, ref Vector3 out_emitterPos, AkReflectionPathInfoArray out_aPaths, out uint io_uArraySize)
	{
		ulong akGameObjectID = GetAkGameObjectID(in_gameObjectID);
		PreGameObjectAPICall(in_gameObjectID, akGameObjectID);
		return (AKRESULT)AkSoundEnginePINVOKE.CSharp_QueryReflectionPaths(akGameObjectID, ref out_listenerPos, ref out_emitterPos, out_aPaths.GetBuffer(), out io_uArraySize);
	}
	...
	public static AKRESULT QueryDiffractionPaths(GameObject in_gameObjectID, ref Vector3 out_listenerPos, ref Vector3 out_emitterPos, AkDiffractionPathInfoArray out_aPaths, out uint io_uArraySize)
	{
		ulong akGameObjectID = GetAkGameObjectID(in_gameObjectID);
		PreGameObjectAPICall(in_gameObjectID, akGameObjectID);
		return (AKRESULT)AkSoundEnginePINVOKE.CSharp_QueryDiffractionPaths(akGameObjectID, ref out_listenerPos, ref out_emitterPos, out_aPaths.GetBuffer(), out io_uArraySize);
	}
	...
	{
		return (AKRESULT)AkSoundEnginePINVOKE.CSharp_SetRoomPortal(in_PortalID, AkTransform.getCPtr(Transform), Extent, bEnabled, FrontRoom, BackRoom);
	}
	...
	{
		return (AKRESULT)AkSoundEnginePINVOKE.CSharp_SetGeometry(in_GeomSetID, Triangles.GetBuffer(), NumTriangles, Vertices, NumVertices, Surfaces.GetBuffer(), NumSurfaces, RoomID, EnableDiffraction, EnableDiffractionOnBoundaryEdges);
	}
