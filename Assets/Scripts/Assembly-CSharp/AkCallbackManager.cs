...
public static class AkCallbackManager
{
	...
	public static AKRESULT Init(InitializationSettings settings)
	{
		...
		m_pNotifMem = ((settings.BufferSize > 0) ? Marshal.AllocHGlobal(settings.BufferSize) : IntPtr.Zero);
			...
			while (intPtr != IntPtr.Zero)
			{
				...
				default:
				{
					...
					case AkCallbackType.AK_MusicPlaylistSelect:
						...
						break;
					default:
						Debug.LogError("WwiseUnity: PostCallbacks aborted due to error: Undefined callback type <" + akCallbackType.ToString() + "> found. Callback object possibly corrupted.");
						return num;
					}
