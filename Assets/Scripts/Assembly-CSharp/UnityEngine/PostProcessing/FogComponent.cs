...
namespace UnityEngine.PostProcessing
{
	public sealed class FogComponent : PostProcessingComponentCommandBuffer<FogModel>
	{
		private static class Uniforms
		{
			...
			internal static readonly int _FogTexBlend = Shader.PropertyToID("_FogTexBlend");

			internal static readonly int _FogTex = Shader.PropertyToID("_FogTex");
		}
		...
		public override void PopulateCommandBuffer(CommandBuffer cb)
		{
			...
			material.SetFloat(Uniforms._FogTexBlend, settings.textureBlend);
			material.SetTexture(Uniforms._FogTex, settings.texture);
			if (settings.texture == null)
			{
				material.EnableKeyword("FOG_COLOR");
				material.DisableKeyword("FOG_TEXTURE");
			}
			else
			{
				material.EnableKeyword("FOG_TEXTURE");
				material.DisableKeyword("FOG_COLOR");
			}
