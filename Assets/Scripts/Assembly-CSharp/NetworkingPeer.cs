...
internal class NetworkingPeer : LoadBalancingPeer, IPhotonPeerListener
{
	...
	private bool m_hasJustReconnected;
	...
	protected internal string AppVersion => string.Format("{0}_{1}", PhotonNetwork.gameVersion, "1.98");
	...
	private bool Reconnect()
	{
		...
		Debug.Log("NetworkingPeer> Reconnect: Started reconnecting...");
		bool flag = false;
		m_hasJustReconnected = false;
		State = ClientState.Disconnected;
		try
		{
			flag = base.Connect(cachedServerAddress, cachedApplicationName, TokenForInit);
		}
		catch (Exception ex)
		{
			Debug.Log("NetworkingPeer> Reconnect: Failed to reconnect " + ex.Message);
			flag = false;
		}
		if (flag)
		{
	...
	protected internal void SetupProtocol(ServerConnection serverType)
	{
		...
		if (base.TransportProtocol != connectionProtocol)
		{
			if (PhotonNetwork.logLevel >= PhotonLogLevel.ErrorsOnly)
			{
				Debug.LogWarning("Protocol switch from: " + base.TransportProtocol.ToString() + " to: " + connectionProtocol.ToString() + ".");
			}
	...
	private bool CallAuthenticate()
	{
		...
		if (AuthMode == AuthModeOption.Auth)
		{
			return OpAuthenticate(AppId, AppVersion, authenticationValues, CloudRegion.ToString(), requestLobbyStatistics);
		}
		return OpAuthenticateOnce(AppId, AppVersion, authenticationValues, CloudRegion.ToString(), EncryptionMode, PhotonNetwork.PhotonServerSettings.Protocol);
	}
	...
	public void OnOperationResponse(OperationResponse operationResponse)
	{
		...
		case 231:
			if (operationResponse.ReturnCode != 0)
			{
				...
				else if (operationResponse.ReturnCode == 32765)
				{
					Debug.LogWarning($"Can't join room. Room full. Disconnecting.");
					SendMonoMessage(PhotonNetworkingMessage.OnConnectionFail, DisconnectCause.GameFull);
				}
				break;
			}
	...
	public void OnStatusChanged(StatusCode statusCode)
	{
		...
		case StatusCode.TimeoutDisconnect:
			Debug.Log("NetworkingPeer> TimeoutDisconnect event: Connection timed out, reconnecting...");
			if (IsInitialConnect)
			{
				if (!_isReconnecting)
				{
	...
	public void OnEvent(EventData photonEvent)
	{
		...
		case 209:
		{
			...
			int viewID2 = obj[0];
			int num3 = obj[1];
			PhotonView photonView2 = PhotonView.Find(viewID2);
			if (photonView2 == null)
			{
				Debug.LogWarning("Can't find PhotonView of incoming OwnershipRequest. ViewId not found: " + viewID2);
				break;
			}
			if (PhotonNetwork.logLevel == PhotonLogLevel.Informational)
			{
				Debug.Log("Ev OwnershipRequest " + photonView2.ownershipTransfer.ToString() + ". ActorNr: " + sender + " takes from: " + num3 + ". local RequestedView.ownerId: " + photonView2.ownerId + " isOwnerActive: " + photonView2.isOwnerActive + ". MasterClient: " + mMasterClientId + ". This client's player: " + PhotonNetwork.player.ToStringFull());
			}
			...
			case OwnershipOption.Takeover:
				if (num3 == photonView2.ownerId || (num3 == 0 && photonView2.ownerId == mMasterClientId) || photonView2.ownerId == 0)
				{
					...
					if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
					{
						Debug.LogWarning(photonView2?.ToString() + " ownership transfered to: " + sender);
					}
		...
		case 210:
		{
			...
			if (photonView != null)
			{
				...
				SendMonoMessage(PhotonNetworkingMessage.OnOwnershipTransfered, photonView, PhotonPlayer.Find(num), PhotonPlayer.Find(ownerId));
			}
			break;
		}
		case 230:
		{
			...
			foreach (object key2 in hashtable.Keys)
			{
				...
				mGameList[text] = new RoomInfo(text, (ExitGames.Client.Photon.Hashtable)hashtable[key2]);
			}
			mGameListCopy = new RoomInfo[mGameList.Count];
			mGameList.Values.CopyTo(mGameListCopy, 0);
			SendMonoMessage(PhotonNetworkingMessage.OnReceivedRoomListUpdate);
		...
		case 229:
		{
			...
			foreach (object key3 in hashtable3.Keys)
			{
				string text2 = (string)key3;
				RoomInfo roomInfo = new RoomInfo(text2, (ExitGames.Client.Photon.Hashtable)hashtable3[key3]);
		...
		case 206:
		{
			...
			byte b2 = b;
			while (b2 - b < hashtable2.Count - num4)
			{
				OnSerializeRead(hashtable2[b2] as object[], photonPlayer, networkTime, correctPrefix);
				b2++;
			}
			break;
		}
		...
		case 203:
			...
			{
				Debug.LogError("Error: Someone else(" + photonPlayer?.ToString() + ") then the masterserver requests a disconnect!");
				break;
			}
			if (_AsyncLevelLoadingOperation != null)
			{
		...
		case 207:
		{
			int num2 = (int)((ExitGames.Client.Photon.Hashtable)photonEvent[245])[(byte)0];
			if (num2 >= 0)
			{
		...
		case 204:
		{
			int key = (int)((ExitGames.Client.Photon.Hashtable)photonEvent[245])[(byte)0];
			PhotonView value = null;
			if (photonViewList.TryGetValue(key, out value))
			{
				RemoveInstantiatedGO(value.gameObject, localOnly: true);
			}
			else if ((int)DebugOut >= 1)
			{
				Debug.LogError("Ev Destroy Failed. Could not find PhotonView with instantiationId " + key + ". Sent by actorNr: " + sender);
			}
			break;
		}
		case 208:
		...
		default:
			...
			{
				Debug.LogWarning("Warning: Unhandled event " + photonEvent?.ToString() + ". Set PhotonNetwork.OnEventCall.");
			}
			break;
		}
	}
	...
	internal void RPC(PhotonView view, string methodName, PhotonTargets target, PhotonPlayer player, bool encrypt, params object[] parameters)
	{
		...
		if (PhotonNetwork.logLevel >= PhotonLogLevel.Full)
		{
			Debug.Log("Sending RPC \"" + methodName + "\" to target: " + target.ToString() + " or player:" + player?.ToString() + ".");
		}
		ExitGames.Client.Photon.Hashtable hashtable = new ExitGames.Client.Photon.Hashtable();
		hashtable[(byte)0] = view.viewID;
		if (view.prefix > 0)
