...
internal class PhotonHandler : MonoBehaviour
{
	...
	private static CloudRegionCode m_bestregion_code;

	internal static CloudRegionCode BestRegionCodeInPreferences
	{
		get
		{
			string text = PlayerPrefs.GetString("PUNCloudBestRegion", "");
	...
	internal IEnumerator PingAvailableRegionsCoroutine(bool connectToBest)
	{
		while (PhotonNetwork.networkingPeer.AvailableRegions == null)
		{
			...
			UnityEngine.Debug.Log("Waiting for AvailableRegions. State: " + PhotonNetwork.connectionStateDetailed.ToString() + " Server: " + PhotonNetwork.Server.ToString() + " PhotonNetwork.networkingPeer.AvailableRegions " + (PhotonNetwork.networkingPeer.AvailableRegions != null));
		...
		if (bestRegion != null)
		{
			...
			UnityEngine.Debug.Log("Found best region: '" + bestRegion.Code.ToString() + "' ping: " + bestRegion.Ping + ". Calling ConnectToRegionMaster() is: " + connectToBest);
		}
		else
		{
			UnityEngine.Debug.LogWarning("Best region currently null!");
		}
		if (connectToBest)
		{
