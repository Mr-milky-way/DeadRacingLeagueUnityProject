...
namespace WebSocketSharp
{
	internal class WebSocketFrame : IEnumerable<byte>, IEnumerable
	{
		...
		private static WebSocketFrame processHeader(byte[] header)
		{
			...
			{
				string message = "A frame has an unsupported opcode.";
				throw new WebSocketException(CloseStatusCode.ProtocolError, message);
			}
			if (!opcode.IsData() && rsv == Rsv.On)
			{
				string message2 = "A non data frame is compressed.";
			...
			if (opcode.IsControl())
			{
				if (fin == Fin.More)
				{
					string message3 = "A control frame is fragmented.";
					throw new WebSocketException(CloseStatusCode.ProtocolError, message3);
				}
				if (b > 125)
				{
					string message4 = "A control frame has too long payload length.";
