...
namespace WebSocketSharp.Net
{
	public sealed class HttpListenerRequest
	{
		...
		public bool IsWebSocketRequest
		{
			get
			{
				if (_httpMethod == "GET" && _protocolVersion > HttpVersion.Version10)
				{
		...
		{
			if (_headers["Transfer-Encoding"] != null)
			{
		...
		private Encoding getContentEncoding()
		{
			...
			HttpUtility.TryGetEncoding(text, out var result);
			return result;
		}
		...
		private RequestStream getInputStream()
		{
			...
			return _connection.GetRequestStream(_contentLength, _chunked);
		}
		...
		internal void SetRequestLine(string requestLine)
		{
			...
			if (!text3.Substring(5).TryCreateVersion(out var result))
			{
				_context.ErrorMessage = "Invalid request line (version)";
				return;
			}
